...
try:
    ...
    for record in data:
        ...
        requiredSkills = tuple((skill["name"], skill["category"]) for skill in record["requiredSkills"])
        ...
        optionalSkills = tuple((skill["name"], skill["category"]) for skill in record["optionalSkills"])
